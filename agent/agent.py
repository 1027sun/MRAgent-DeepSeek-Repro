...
def _completed_sessions(rewrite_path: str) -> set:
    """Session keys that already have a well-formed record in the rewrite JSONL.

    Used by rewrite_sample() for per-session resume: a single degenerate session can be
    regenerated without redoing (and re-paying for) the whole conversation. A record
    only counts as done when it carries a non-empty `sentence` list, so the malformed
    output a repetition loop produces is treated as missing and retried.
    """
    done = set()
    ...
    with open(rewrite_path, encoding="utf-8") as fh:
        for line in fh:
            ...
            for key, val in rec.items():
                if isinstance(val, dict) and isinstance(val.get("sentence"), list) and val["sentence"]:
                    done.add(key)
    return done
...
class Agent:
    ...
    def answer_question(self, question: str, category=0, question_emb=None, override_question_time=None, lm_current_date=None) -> Dict[str, Any]:
        ...
        # guard: the keyword-extraction call can return None (JSON parse failed every
        # retry) or a dict whose "keywords" is null. Neither should lose a whole question.
        question_keys = self.extract_question_keys(question) or {}
        question_keywords = question_keys.get("keywords") or []
        self.memory_controller.set_queried_keywords(question_keywords)
        query_answers = self.memory_controller.evaluate_relations_over_graph(question_keywords)
    ...
    def rewrite_sample(self, sample: dict, rewrite_path: str, session_id_ref: int = 0):
        # per-session resume: sessions already written and well-formed are skipped
        done = _completed_sessions(rewrite_path)
        if done:
            logger.info(f"rewrite resume: {len(done)} session(s) already on disk, skipping")
        ...
        if config.DATASET == "LM":
            ...
            for batch_start in range(0, len(session_items), batch_size):
                ...
                if all(sid in done for sid in batch_session_ids):
                    i += 1
                    continue
            ...
            for attempt in range(1, max_tries + 1):
                ...
                if flag:
                    ...
                    if attempt == max_tries:
                        n_before = len(keys_out.get("sentence", [])) if isinstance(keys_out, dict) else 0
                        flag, err = json_scheme.check_key_json(keys_out, text, replace=True)
                        n_after = len(keys_out.get("sentence", [])) if isinstance(keys_out, dict) else 0
                        logger.warning(
                            "keyword JSON still invalid after %d tries (last error: %s); "
                            "repair mode kept %d/%d sentences",
                            max_tries, last_err, n_after, n_before,
                        )
    ...
    def store_event_new(self, events, keys,  session_id):
        ...
        for ps in (personal_sentences or []):
