...
def embed_sample(qa_list, rewrite_path, FILE_EMBEDDING):
    ...
    for j in range(len(record_rewrite)):
        ...
        # guard: a degenerate rewrite (repetition loop) can emit a flat sentence object
        # instead of the full record. Skip it here rather than crashing on `for s in None`.
        if not isinstance(sentences, list):
            logger.warning(
                "embed_sample: record %d has no usable `sentence` list (keys=%s); skipping session",
                j, sorted(session_data) if isinstance(session_data, dict) else type(session_data).__name__,
            )
            continue
        sentence_list = []
        for s in sentences:
