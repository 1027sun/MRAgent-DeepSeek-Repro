...
def get_question(dataset, agent, question_list, sample_id, memory, result_path, question_embeddings=None):
    ...
    if config.maxq:  # optional cap for cheap pilot runs (--maxq N)
        qa_list = qa_list[:config.maxq]
...
def main():
    ...
    for sample_id, sample in conversation_list.items():
        ...
        with per_sample_log(sample_id=sample_id, dataset=dataset):
            ...
            # Always call rewrite_sample: it resumes per session internally, so records
            # that are missing or malformed (e.g. after a repetition-loop failure) get
            # regenerated while the 18 good sessions are reused for free.
