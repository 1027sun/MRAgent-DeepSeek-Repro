...
def _record_usage(model: str, resp) -> None:
    """Append one JSONL line per API call so a run's token cost can be audited.

    Never raises: bookkeeping must not be able to break an experiment.
    """
    try:
        ...
        for key in ("prompt_cache_hit_tokens", "prompt_cache_miss_tokens", "cached_tokens"):
            if key in extra:
                rec[key] = extra[key]
        path = getattr(config, "USAGE_LOG", None)
        if not path:
            return
        with _USAGE_LOCK:
        ...
        logger.debug("usage logging failed: %r", exc)
...
class LLM:
    ...
    def chat_with_tool(
        ...
        # provider quirks: strip request keys this endpoint rejects, and merge any
        # extra body it needs. DeepSeek ignores/refuses `seed` and
        # `parallel_tool_calls`, and enables thinking mode by default, so a DeepSeek
        # run sets LLM_DROP_PARAMS and LLM_EXTRA_BODY in .env.
        for _key in config.LLM_DROP_PARAMS:
            req.pop(_key, None)
        if config.LLM_EXTRA_BODY:
            _body = dict(req.get("extra_body") or {})
            _body.update(config.LLM_EXTRA_BODY)
            req["extra_body"] = _body
