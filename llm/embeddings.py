...
# The embedding endpoint is configured independently of the chat endpoint, so chat can
# run on DeepSeek while embeddings go through OpenRouter / OpenAI / a relay.
EMBED_API_KEY = os.getenv("EMBED_API_KEY") or os.getenv("OPENROUTER_API_KEY")
EMBED_BASE_URL = os.getenv("EMBED_BASE_URL", "https://openrouter.ai/api/v1").rstrip("/")
